...
//    -lr recursively lists the contents of the Nachos directory
//    -D prints the contents of the entire file system
//    -DS prints -D with Sectors data in directory field
//
...
{
    // MP4 Assignment
    if (!kernel->fileSystem->Create(name, -1)) // Use default directory size
        printf("Create Directory: unable to create directory %s\n", name);
}
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-u") == 0)
        {
            ...
            cout << "Partial usage: nachos [-l] [-D] [-S] [-F]\n";
#endif //FILESYS_STUB
        }
    }
    ...
    if (dumpFlag)
    {
        kernel->fileSystem->Print(printSector, printFreeMap);
    }
    if (dirListFlag)
    {
