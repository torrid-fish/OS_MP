...
void Directory::Print(int depth, bool printSector)
{
    ...
    for (int i = 0; i < tableSize; i++)
        if (table[i].inUse)
        {
            ...
            for (int j = 0; j <= 4 * depth; j++) cout << " |"[j%4==0];
            printf("- [%c] %s\n", "FD"[table[i].isDirectory], table[i].name);
            hdr->FetchFrom(table[i].sector);
            hdr->Print(depth, table[i].sector, printSector);
            
            if (table[i].isDirectory) {
                Directory* subDirectory = new Directory(64); // Default Directory size
                OpenFile* subDirectoryFile = new OpenFile(table[i].sector);
                subDirectory->FetchFrom(subDirectoryFile);
                subDirectory->Print(depth + 1, printSector);
                delete subDirectory;
