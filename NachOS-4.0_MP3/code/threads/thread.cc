...
Thread::~Thread()
{
    ...
    // Actually free space
    if (space != NULL)
        DEBUG(dbgThread, "Deleting space of thread: " << name);
        delete space;
...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;
    // Update approximated burst time when thread become waiting
    double burstTime = approximatedBurstTime; 
    // Update T
    updateT(kernel->scheduler->RunTime(), kernel->stats->totalTicks);
...
void ThreadPrintL1(Thread *t) { t->PrintL1(); }
#ifdef PARISC
